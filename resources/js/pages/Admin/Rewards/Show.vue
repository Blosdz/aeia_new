...
import { ArrowLeft, DollarSign, Percent, TrendingUp, AlertCircle, CheckCircle2, Users } from 'lucide-vue-next';
...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: `${props.reward.client_name} - ${props.reward.fund_name}`, href: route('admin.rewards.show', props.reward.id) },
];
...
<template>
  <Head :title="`Reward - ${reward.client_name}`" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6 p-6">
      ...
      <div class="flex justify-between items-start">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-muted-foreground mt-1">Cliente: {{ reward.client_name }} ({{ reward.client_email }})</p>
          </div>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-sm">Información del Fondo</CardTitle>
        </CardHeader>
        <CardContent>
          <p>{{ reward.fund_description }}</p>
        </CardContent>
      </Card>
      ...
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card class="border-2 border-blue-500">
          ...
          <CardContent>
            ...
            <p class="text-xs text-muted-foreground mt-1">A retornar al cliente</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Detalle paso a paso de cómo se calcula este reward</CardDescription>
        </CardHeader>
        <CardContent>
          <div class="space-y-4">
            <!-- Paso 1: Inversión -->
            <div class="border rounded-lg p-4 bg-gradient-to-r from-blue-50 to-transparent dark:from-blue-950/20">
              ...
              <p class="text-sm text-muted-foreground">El monto que el cliente invirtió en el fondo</p>
            </div>
            ...
            <div class="border rounded-lg p-4 bg-gradient-to-r from-blue-50 to-transparent dark:from-blue-950/20">
              ...
              <p class="text-sm text-muted-foreground">Ganancia que recibirá el cliente</p>
            </div>

            <!-- Resultado Final -->
            <div class="border-2 border-green-500 rounded-lg p-4 bg-green-50 dark:bg-green-950/20">
              <div class="flex justify-between items-center">
                <p class="font-bold text-lg">TOTAL A RETORNAR AL CLIENTE</p>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent class="space-y-3">
            <div class="flex justify-between items-center p-3 border rounded bg-muted/50">
              <span class="font-medium">Nombre:</span>
              <span>{{ reward.client_name }}</span>
            </div>
            <div class="flex justify-between items-center p-3 border rounded bg-muted/50">
              <span class="font-medium">Email:</span>
              <span class="text-sm">{{ reward.client_email }}</span>
            </div>
            <div v-if="reward.was_referred && reward.referrer_name" class="flex justify-between items-center p-3 border rounded bg-blue-50 dark:bg-blue-950/20">
              <span class="font-medium">Referido por:</span>
              <span class="font-semibold">{{ reward.referrer_name }}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent class="space-y-3">
            ...
            <div v-if="reward.paid_at" class="flex justify-between items-center p-3 border rounded bg-green-50 dark:bg-green-950/20">
              ...
              <span class="text-green-600 dark:text-green-400 font-semibold">{{ formatDate(reward.paid_at) }}</span>
            </div>
            <div v-else-if="reward.status === 'closed'" class="flex justify-between items-center p-3 border rounded bg-yellow-50 dark:bg-yellow-950/20">
              ...
              <span class="text-yellow-600 dark:text-yellow-400 font-semibold">Esperando procesamiento de pago</span>
            </div>
          </CardContent>
        </Card>
      </div>
